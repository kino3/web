...
SIG
    ...
    unless ($@) {
        if (lc $enc ne 'iso-8859-1' && $hdrs->{Subject} =~ /[^[:print:]]/ ) {
            $hdrs->{Subject} = MIME::Words::encode_mimeword($hdrs->{Subject}, 'b', $mail_enc);
        }
    }
...
sub _send_mt_smtp {
    ...
    return $class->error(MT->translate(
        "Sending mail via SMTP requires that your server have Mail::Sendmail installed: [_1]", $@ )) if $@;
    my %hdrs = %$hdrs;
    $hdrs{Message} = $body;
