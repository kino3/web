...
# xxx Expire sessions on shorter timeout?
    ...
    my $username = MT::I18N::encode_text($auth->{Username},undef,$enc);
...
sub get_weblogs {
    ...
    while (my $thing = $iter->()) {
        ...
        my $blogname = MT::I18N::encode_text($blog->name, undef, 'utf-8');
...
sub new_post {
    ...
    $entry->title(MT::I18N::encode_text($atom->title,'utf-8',$enc));
    $entry->text(MT::I18N::encode_text($atom->content()->body(),'utf-8',$enc));
    $entry->excerpt(MT::I18N::encode_text($atom->summary,'utf-8',$enc));
...
sub get_posts {
    ...
    while (my $entry = $iter->()) {
        ...
        $e->add_link({ rel => 'service.edit', type => 'application/x.atom+xml',
                       href => ($uri . $entry->id), title => MT::I18N::encode_text($entry->title, undef,'utf-8') });
        $feed->add_entry($e);
    }
...
sub get_categories {
    ...
    while (my $cat = $iter->()) {
        my $catlabel = MT::I18N::encode_text($cat->label, undef, 'utf-8');
        if (LIBXML) {
