...
sub _nextprev {
    ...
    $direction = $direction eq 'next' ? 'ascend' : 'descend';

    unless ($obj->{$label} || !$obj->created_on) {
        $obj->{$label} = $class->load({
            blog_id => $obj->blog_id,
...
sub get_excerpt {
    ...
    my $blog = $entry->blog() || return $entry->error(MT->translate(
                                                     "Load of blog failed: [_1]",
                                                     MT::Blog->errstr));
    MT::I18N::first_n($excerpt, $words || $blog->words_in_excerpt || MT::I18N::const('DEFAULT_LENGTH_ENTRY_EXCERPT')) . '...';
}
